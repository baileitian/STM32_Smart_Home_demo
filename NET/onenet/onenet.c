...
//	说明：		
//==========================================================
extern int liv_LED_flag;
extern int bed_LED_flag;
extern int illumination_flag ;
extern int fanflag;
extern void access_control(int mode);
extern void Fan_speed_set(int speed_level);//风扇调速 0,1,2,3
	...
	switch(type)
	{
		...
		case MQTT_PKT_PUBLISH:														//接收的Publish消息
			...
			if(result == 0)
			{
				...
				if (!json)UsartPrintf("Error before: [%s]\n",cJSON_GetErrorPtr());
				else
				{
					json_value = cJSON_GetObjectItem(json , "target");
					UsartPrintf("json_value: [%s]\n",json_value->string);
					UsartPrintf("json_value: [%s]\n",json_value->valuestring);
					if(strstr(json_value->valuestring,"LED1") != NULL)//客厅灯
					{
					...
					if(strstr(json_value->valuestring,"LED2") != NULL)//老人房灯
					{
						json_value = cJSON_GetObjectItem(json , "value");
						if(json_value->valueint == 1)
						{
					...
					if(strstr(json_value->valuestring,"window") != NULL)//窗帘
					{
						json_value = cJSON_GetObjectItem(json , "value");
						if(json_value->valueint == 1)
						{
							if(illumination_flag == 0)//开窗帘
							{
									printf("window_ON\r\n");
									access_control(1);
									illumination_flag = 1;
					...
					if(strstr(json_value->valuestring,"fan") != NULL)//窗帘
					{
						json_value = cJSON_GetObjectItem(json , "value");
						...
						if(json_value->valueint == 2)
						{
							printf("Fan 2\r\n");
							Fan_speed_set(2);
							fanflag = 2;
						}
						if(json_value->valueint == 3)
						{
							printf("Fan 3\r\n");
							Fan_speed_set(3);
							fanflag = 3;
