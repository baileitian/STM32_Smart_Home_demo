...
void GBK_LibFont_Test(void)
{
	...
	DrawFont_GBK16B(0,16,RED,"2:GBK字库--显示测试");
	
	delay_ms(1000);
	
	LCD_Clear(WHITE);
	
	GBK_Show_Str(0,0,lcddev.width,32,"十种字库",32,D_Color,WHITE,0);	
	GBK_Show_Str(0,40,lcddev.width,16,"设计：大越电子",16,D_Color,B_Color,0);	
	GBK_Show_Str(0,60,lcddev.width,16,"From: mcudev.taobao.com",16,D_Color,B_Color,0);
	
	
	DrawFont_GBK12B(0,80,BLUE,"Date: 2019/05/08");
	
	GBK_Show_StrMid(0,96,12,lcddev.width,BLUE,WHITE,"居中显示函数: ");//居中显示	
	GBK_Show_StrMid(0,110,12,lcddev.width,BLUE,WHITE,"2019/05/08");//居中显示	

	delay_ms(1000);
  LCD_Clear(WHITE);
	
	GBK_Show_Str(0,20,240,12,"汉字12x12-GBK字库：",12,D_Color,B_Color,0);	//显示内码低字节
				
				
	GBK_Show_Str(0,40,240,16,"汉字16x16字库：",16,D_Color,B_Color,0);	//显示内码低字节
				

	GBK_Show_Str(0,60,240,24,"24x24字库：",24,D_Color,B_Color,0);	//显示内码低字节
				
				
	DrawFont_GBK32B(0,90,BLUE,"32x32字:");	//显示内码低字节

	delay_ms(1000);
  LCD_Clear(WHITE);
	
	DrawFont_GBK16B(0,24,BLUE,"ASCII库:");
	//GBK_Show_Str(16,220,240,48,"48",48,D_Color,B_Color,0);
  DrawFontASC_GBK48B(64,0,BLUE,"48");

  //GBK_Show_Str(120,220,240,64,"64",64,D_Color,B_Color,0);	
	DrawFontASC_GBK64B(32,48,BLUE,":64");
